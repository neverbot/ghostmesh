...
<template>
  <Teleport to="body">
    ...
    >
      <div
        ref="menuEl"
        class="fixed min-w-48 rounded-lg border border-slate-700 bg-slate-800 py-1 shadow-xl"
      ...
      >
        ...
        >
          ...
          {{ prefs.isPreviewHidden(nick) ? '✓ Previews hidden' : 'Hide previews' }}
        </button>
        ...
        >
          <svg
            class="h-3.5 w-3.5"
            :class="prefs.isUserHidden(nick) ? 'text-red-400' : 'text-slate-400'"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="1.5"
          ...
          {{ prefs.isUserHidden(nick) ? '✓ Messages hidden' : 'Hide messages' }}
        </button>
      </div>
    </div>
  </Teleport>
